...
public class DatabaseFactory {
    ...
    public static SqlScriptFactory createSqlScriptFactory(final JdbcConnectionFactory jdbcConnectionFactory,
        ...
        return (resource, mixed, resourceProvider) -> new ParserSqlScript(createParser(jdbcConnectionFactory, configuration, parsingContext), resource, getMetadataResource(resourceProvider, resource), mixed);
    }
    ...
    public static SqlScriptExecutorFactory createSqlScriptExecutorFactory(final JdbcConnectionFactory jdbcConnectionFactory) {
        ...
        if (DatabaseType.ORACLE == databaseType) {
            return connection -> new OracleSqlScriptExecutor(new JdbcTemplate(connection, databaseType));
        }
        if (DatabaseType.DM == databaseType) {
            return connection -> new DMSqlScriptExecutor(new JdbcTemplate(connection, databaseType)
            );
        }
        return connection -> new DefaultSqlScriptExecutor(new JdbcTemplate(connection, databaseType));
    }
